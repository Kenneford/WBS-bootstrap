...
function getAuthors() {
  return pool.query(`SELECT * FROM author;`).then((data) => {
    return data.rows;
  });
}

function getOneBook(id) {
  return pool.query("SELECT * FROM books WHERE id=$1;", [id]).then((data) => {
    return data.rows;
  });
}

function deleteAuthor(id) {
  return pool.query("DELETE FROM author where id=$1;", [id]).then((data) => {
    return data.rows;
  });
}

function deleteBook(id) {
  return pool.query("DELETE FROM books where id=$1;", [id]).then((data) => {
    return data.rows;
  });
}
